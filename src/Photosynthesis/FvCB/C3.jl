...
Base.@kwdef mutable struct C3{T <: Real} <: C3Type
    ...
    S_Jmax::T = 650.0 # Entropy term for Jmax (J/K/mol)
    ...
    S_TPU::T = 650.0 # Entropy term for TPU (J/K/mol)
    # Respiration
    ...
    S_gm::T = 1400.0 # Entropy term for gm (J/K/mol)
...
function photosynthesis(p::C3Type, PAR, RH, Tleaf, Ca, O2, gb, net)
    ...
    TPU = peaked(p.TPU25, p.E_TPU, p.D_TPU, p.S_TPU, Tleaf) # μmol/m2/s
